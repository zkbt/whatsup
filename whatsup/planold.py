...
from population import Known, Interesting
...
class Plan(Talker):
    def __init__(self, semester='2015B', observatory='LCO', maxairmass=2.5, maxsun=-12.0, directory='/Users/zkbt/Dropbox/proposals/observability/', name='default'):
        ...
        self.semester = Semester(semester, plan=self)
    ...
    def loadPopulation(self):
        ...
        # tidy up junk
        ok = (self.known.J > 0)*(self.known.a_over_r > 0)*(self.known.dec*astropy.units.deg < self.observatory.latitude + 60.0*astropy.units.deg)*(self.known.dec*astropy.units.deg > self.observatory.latitude - 60.0*astropy.units.deg)
    ...
    def plotTransits(self):
        ...
        self.figure = plt.figure('upcoming transits', figsize=(10, 3))
        self.gs = plt.matplotlib.gridspec.GridSpec(2,1, hspace=0, wspace=0, height_ratios=[0.3, 1.0], bottom=0.3)
        self.ax = {}
    ...
    def movie(self, fps=10, bitrate=10000):
